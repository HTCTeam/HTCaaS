...
public class CancelMetaJob {

	final static Logger logger = LoggerFactory.getLogger(CancelMetaJob.class);
...
//				formatter.printHelp("CancelMetaJob", options);
				String help= "htcaas-job-cancel [OPTIONS] ";
				String syn= "\nhtcaas-job-cancel [-m <metajob id>]";
				syn += "\n\nReport bugs to <htcaas-admin@kisti.re.kr>.";
				formatter.printHelp(250, help, "Cancels a metajob.\n", options, syn);				
				System.exit(0);
			} else {
				if (cmd.hasOption("m")) {
...
//			e.printStackTrace();
		...
		int num = jmClient.cancelMetaJob(user, Integer.parseInt(metaJobId));
		if (num >= 0){
			System.out.println(metaJobId);
		}
		else
			System.out.println("The cancel of the metajob is failed.! MetaJob ID:" + metaJobId);
	}
